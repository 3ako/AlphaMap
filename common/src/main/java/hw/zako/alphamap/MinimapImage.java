...
// Миникарта собирается в одну текстуру на процессоре и уходит на экран одним квадом: форма, поворот
// и зум — это выборка пикселей, а не лишние примитивы, так что видеокарте всё равно, что нарисовано.
// Пересборка идёт, только когда картинка реально поехала: стоя на месте кадр не стоит ничего.
...
final class MinimapImage {
    ...
    private boolean fresh(int revision, int size, double centerX, double centerZ,
        ...
        // Потолок частоты пересборки: на высоком fps картинка и так не успевает измениться на пиксель.
    ...
    private void paint(AtlasClient atlas, AtlasGeometry geometry, double centerX, double centerZ,
        ...
        // Тайл под соседними пикселями почти всегда тот же самый, поэтому держим последний найденный.
