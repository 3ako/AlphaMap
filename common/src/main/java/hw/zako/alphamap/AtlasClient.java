...
public final class AtlasClient {
    ...
    // Растёт на каждую смену картинки атласа: по нему миникарта понимает, что пора пересобраться.
    @NonFinal
    @Getter
    int revision;
    @NonFinal
    ...
    // Сколько раз спросили в пустоту. Сервер, который вообще не отвечает, дальше не беспокоим.
    @NonFinal
    int unanswered;
    ...
    // Сервер объявил канал карты. Пока это не так, мод не ходит в сеть и не трогает ввод.
