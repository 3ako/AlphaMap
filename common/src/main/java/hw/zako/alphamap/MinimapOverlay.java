...
public final class MinimapOverlay {
    ...
    // Экран расположения показывает миникарту поверх затемнения и до того, как её включили.
    public void preview(Canvas canvas, float partialTick) {
        ...
        // Карты на сервере нет, но место под миникарту выбрать всё равно надо — показываем габарит.
        int size = settings.minimapSize();
    ...
    private void paint(Canvas canvas, Minecraft client, float partialTick) {
        ...
        // Взгляд берём сглаженным между тиками, иначе карта дёргается двадцать раз в секунду.
    ...
    private void marks(Canvas canvas, Minecraft client, float centreX, float centreY,
        ...
        // Сущности к краю не прижимаем: они бегают, и кайма мигала бы точками.
    ...
    // Коэффициент прижатия к краю: 1 — объект попал в миникарту, меньше — он за краем и его
    // подтягивают к кайме, чтобы осталось видно направление.
    ...
    // Стрелка рисуется рядами под повёрнутой матрицей: отдельная текстура и её загрузка не нужны.
    ...
    private void coordinates(Canvas canvas, Minecraft client, int left, int top, int size) {
        ...
        // У нижнего края экрана подпись не влезает под миникартой — уводим её наверх.
