...
// Список типов строится из того, что прислал сервер, плюс из того, что игрок когда-то трогал:
// новая структура на сервере появляется здесь сама, а старый выбор не пропадает.
public final class MarkerListScreen extends Screen {
    ...
    private Component label(String icon) {
        // Незнакомому типу перевода нет — показываем его собственный идентификатор.
